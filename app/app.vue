...
/*
 * Was am letzten abgeschlossenen Monat galt, damit der Pfeil sagen kann, wohin die Stadt geht.
 *
 * Hier stand jahrelang ein toter Hinweis. Die alte Fassung schrieb beim Monatswechsel
 * `previousSupport = { ...snapshot.support }` — wenn dieser Watcher läuft, **ist** der Schnappschuss
 * aber schon der neue. Verglichen wurde jeder Monat mit sich selbst, die Differenz war immer null,
 * und der Pfeil ist in keiner Kampagne je erschienen. Gemeldet hat es niemand: es fehlte nichts, es
 * stand nur nie etwas da.
 *
 * Der Griff, der fehlte, ist `handOver` — was gerade galt, wird zum Vorherigen, und erst dann kommt
 * der neue Stand herein. Siehe `app/utils/drift.ts`, wo der Fehler selbst als Testfall steht.
 */
const settledSupport = ref<Record<string, number> | null>(null)
const previousSupport = ref<Record<string, number> | null>(null)
watch(() => snapshot.value?.month, () => {
  const support = snapshot.value?.support
  if (!support)
    return
  const next = handOver(settledSupport.value, { ...support })
  previousSupport.value = next.previous
  settledSupport.value = next.settled
})

/** Welche Richtung seit dem letzten abgeschlossenen Monat, als −1, 0 oder 1. */
const supportDrift = computed(() => {
  const id = selectedPartyId.value
  return id ? drift(ownSupport.value, previousSupport.value?.[id]) : 0
})
