...
export const EVENTS: EventDefinition[] = [
  ...
  /* ══ Weiche Investorenpaket: Vorkauf gezogen ═══════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'hou-preempt-stock',
    kind: 'decision',
    category: 'housing',
    title: 'Vierhundert Wohnungen wollen verwaltet werden',
    briefing:
      'Der Vorkauf ist vollzogen, und seitdem gehören der Stadt vierhundert Wohnungen mit Instandhaltungsstau. Eine eigene Verwaltung aufzubauen dauert zwei Jahre; ein Dienstleister wäre morgen da und nimmt sieben Prozent.',
    urgency: 'normal',
    trigger: { earliestMonth: 14, latestMonth: 122, conditions: [], baseWeight: 8, cooldownMonths: 40, oncePerCampaign: true, requiresChoiceIds: ['hou-investor-block:hou-investor-preempt'] },
    immediateEffects: [],
    defaultOptionId: 'hou-preempt-contract',
    options: [
      {
        id: 'hou-preempt-own',
        label: 'Eigene Verwaltung aufbauen',
        rationale: 'Zwei Jahre Aufbau, danach bleibt das Wissen im Haus.',
        oneOffCost: 4.2,
        monthlyCost: 0.28,
        axes: { marketVsPublic: -0.8, fiscalRestraint: -0.4, redistribution: 0.5 },
        salience: { marketVsPublic: 0.9 },
        effects: [effect({ target: 'socialUnits', expected: 180, delayMonths: 16, rampMonths: 20, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'hou-preempt-contract',
        label: 'An einen Dienstleister vergeben',
        rationale: 'Sofort besetzt, sieben Prozent vom Mietertrag, und kündbar.',
        oneOffCost: 0,
        costMonths: 84,
        monthlyCost: 0.19,
        axes: { marketVsPublic: 0.5, fiscalRestraint: 0.4 },
        salience: { marketVsPublic: 0.7 },
        effects: [effect({ target: 'socialUnits', expected: 95, delayMonths: 4, rampMonths: 10, confidence: 'high' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Investorenpaket: Verkauf gelaufen ═══════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'hou-investor-second',
    kind: 'decision',
    category: 'housing',
    title: 'Derselbe Investor kauft das nächste Paket',
    briefing:
      'Nach den vierhundert stehen dreihundert weitere zum Verkauf, vom selben Verkäufer an denselben Käufer. Beim ersten Mal hat die Stadt nichts getan; dass sie diesmal etwas tut, erwartet niemand mehr.',
    urgency: 'important',
    trigger: { earliestMonth: 26, latestMonth: 124, conditions: [{ metric: 'averageRent', operator: '>', value: 13 }], baseWeight: 9, cooldownMonths: 36, oncePerCampaign: true, requiresChoiceIds: ['hou-investor-block:hou-investor-none'] },
    immediateEffects: [],
    defaultOptionId: 'hou-second-none',
    options: [
      {
        id: 'hou-second-preempt',
        label: 'Diesmal den Vorkauf ziehen',
        rationale: 'Teurer als beim ersten Mal, weil der Markt weitergelaufen ist.',
        oneOffCost: 21,
        monthlyCost: 0.24,
        axes: { marketVsPublic: -0.8, redistribution: 0.7, fiscalRestraint: -0.7 },
        salience: { marketVsPublic: 1, fiscalRestraint: 0.8 },
        effects: [effect({ target: 'socialUnits', expected: 300, delayMonths: 5, rampMonths: 12, confidence: 'high' })],
        sourceIds: MODEL,
      },
      {
        id: 'hou-second-none',
        label: 'Wieder laufen lassen',
        rationale: 'Der Haushalt bleibt unberührt. Der Bestand auch.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { marketVsPublic: 0.7, fiscalRestraint: 0.6 },
        salience: { marketVsPublic: 0.8 },
        effects: [effect({ target: 'homelessPeople', expected: 140, delayMonths: 8, rampMonths: 16, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 2,
  ...
  /* ══ Weiche offene Szene: Druckraum ════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'saf-room-neighbours',
    kind: 'decision',
    category: 'safety',
    title: 'Die Nachbarschaft klagt gegen den Druckraum',
    briefing:
      'Die Zahlen am Bahnhof sind besser, die Anwohnerinitiative trotzdem vor Gericht. Sie will den Betrieb auf Tagesstunden begrenzt sehen. Das Sozialamt sagt, nachts wäre er am nötigsten.',
    urgency: 'important',
    trigger: { earliestMonth: 28, latestMonth: 126, conditions: [], baseWeight: 8, cooldownMonths: 34, oncePerCampaign: true, requiresChoiceIds: ['saf-drug-scene:saf-drug-consumption-room'] },
    immediateEffects: [],
    defaultOptionId: 'saf-room-limit',
    options: [
      {
        id: 'saf-room-defend',
        label: 'Den Nachtbetrieb verteidigen',
        rationale: 'Prozesskosten und ein Konflikt im Viertel, der bleibt.',
        oneOffCost: 1.4,
        monthlyCost: 0,
        axes: { securityAuthority: -0.6, opennessIntegration: 0.7, redistribution: 0.5 },
        salience: { opennessIntegration: 0.9, securityAuthority: 0.7 },
        effects: [effect({ target: 'integrationPlaces', expected: 90, delayMonths: 3, rampMonths: 10, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'saf-room-limit',
        label: 'Auf Tagesstunden begrenzen',
        rationale: 'Ruhe im Viertel. Nachts steht die Szene wieder draußen.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { securityAuthority: 0.6, opennessIntegration: -0.5 },
        salience: { securityAuthority: 0.8 },
        effects: [effect({ target: 'orderServiceFte', expected: -6, delayMonths: 4, rampMonths: 10, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche offene Szene: Präsenz ══════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'saf-presence-displaced',
    kind: 'decision',
    category: 'safety',
    title: 'Die Szene steht jetzt im Grünzug',
    briefing:
      'Der Bahnhofsvorplatz ist frei. Zwei Kilometer weiter, im Grünzug hinter der Universität, steht dieselbe Gruppe — nur ohne Toiletten, ohne Beratung und ohne Licht.',
    urgency: 'important',
    trigger: { earliestMonth: 28, latestMonth: 126, conditions: [], baseWeight: 9, cooldownMonths: 30, oncePerCampaign: false, requiresChoiceIds: ['saf-drug-scene:saf-drug-presence'] },
    immediateEffects: [],
    defaultOptionId: 'saf-displaced-again',
    options: [
      {
        id: 'saf-displaced-service',
        label: 'Aufsuchende Hilfe in den Grünzug',
        rationale: 'Ein Bus, zwei Sozialarbeiterinnen, dort wo die Leute sind.',
        oneOffCost: 1.1,
        monthlyCost: 0.21,
        costMonths: 96,
        axes: { opennessIntegration: 0.7, redistribution: 0.6, securityAuthority: -0.5 },
        salience: { opennessIntegration: 0.8, redistribution: 0.6 },
        effects: [effect({ target: 'integrationPlaces', expected: 120, delayMonths: 5, rampMonths: 14, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'saf-displaced-again',
        label: 'Auch dort räumen',
        rationale: 'Kostet nichts Neues und verschiebt das Problem ein weiteres Mal.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { securityAuthority: 0.8, opennessIntegration: -0.6 },
        salience: { securityAuthority: 0.9 },
        effects: [effect({ target: 'greenSpaceHectares', expected: -3, delayMonths: 6, rampMonths: 12, confidence: 'low' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Rechenzentrum: zugesagt ════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'eco-datacenter-power',
    kind: 'decision',
    category: 'environment',
    title: 'Das Rechenzentrum braucht ein eigenes Umspannwerk',
    briefing:
      'Die zugesagte Leistung übersteigt, was das Netz im Osten hergibt. Der Betreiber zahlt die Hälfte des Umspannwerks, die andere Hälfte will er von der Stadt — oder er drosselt und stellt weniger Leute ein.',
    urgency: 'important',
    trigger: { earliestMonth: 30, latestMonth: 124, conditions: [], baseWeight: 8, cooldownMonths: 40, oncePerCampaign: true, requiresChoiceIds: ['eco-datacenter:eco-datacenter-accept', 'eco-datacenter:eco-datacenter-conditions'] },
    immediateEffects: [],
    defaultOptionId: 'eco-power-throttle',
    options: [
      {
        id: 'eco-power-pay',
        label: 'Die Hälfte übernehmen',
        rationale: 'Neun Millionen für eine Anlage, die danach allen gehört.',
        oneOffCost: 9,
        monthlyCost: 0,
        axes: { growthVsPreservation: 0.7, marketVsPublic: 0.4, fiscalRestraint: -0.6 },
        salience: { growthVsPreservation: 0.9, fiscalRestraint: 0.7 },
        effects: [effect({ target: 'businessSites', expected: 190, delayMonths: 12, rampMonths: 20, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'eco-power-throttle',
        label: 'Ihn drosseln lassen',
        rationale: 'Kein Geld aus dem Haushalt, und ein kleineres Rechenzentrum.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { fiscalRestraint: 0.8, growthVsPreservation: -0.4 },
        salience: { fiscalRestraint: 0.9 },
        effects: [effect({ target: 'businessSites', expected: 30, delayMonths: 10, rampMonths: 16, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Rechenzentrum: abgelehnt ═══════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'eco-datacenter-elsewhere',
    kind: 'decision',
    category: 'economy',
    title: 'Der Betreiber baut im Nachbarkreis',
    briefing:
      'Zwanzig Kilometer weiter steht die Halle jetzt. Die Gewerbesteuer geht dorthin, die Pendlerinnen kommen von hier, und die freigehaltene Fläche im Osten liegt weiter brach.',
    urgency: 'normal',
    trigger: { earliestMonth: 30, latestMonth: 124, conditions: [], baseWeight: 7, cooldownMonths: 42, oncePerCampaign: true, requiresChoiceIds: ['eco-datacenter:eco-datacenter-reject'] },
    immediateEffects: [],
    defaultOptionId: 'eco-elsewhere-hold',
    options: [
      {
        id: 'eco-elsewhere-park',
        label: 'Die Fläche als Gewerbepark erschließen',
        rationale: 'Straße, Kanal, Glasfaser — und danach kommen kleinere Betriebe statt eines großen.',
        oneOffCost: 12.5,
        monthlyCost: 0.16,
        costMonths: 48,
        axes: { growthVsPreservation: 0.6, marketVsPublic: -0.3, fiscalRestraint: -0.5 },
        salience: { growthVsPreservation: 0.8 },
        effects: [effect({ target: 'businessSites', expected: 230, delayMonths: 14, rampMonths: 24, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'eco-elsewhere-hold',
        label: 'Die Fläche freihalten',
        rationale: 'Sie bleibt, was sie ist, und kostet nichts.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { growthVsPreservation: -0.8, climateAmbition: 0.5, fiscalRestraint: 0.5 },
        salience: { growthVsPreservation: 0.9, climateAmbition: 0.6 },
        effects: [effect({ target: 'greenSpaceHectares', expected: 6, delayMonths: 8, rampMonths: 18, confidence: 'high' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Anwohnerparken: Kostendeckung ══════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'mob-parking-backlash',
    kind: 'decision',
    category: 'mobility',
    ...
    immediateEffects: [],
    defaultOptionId: 'mob-backlash-hold',
    options: [
      {
        id: 'mob-backlash-hour',
        label: 'Erste Stunde frei',
        rationale: 'Ein Zugeständnis, das die Hälfte der Einnahmen kostet und den Konflikt beendet.',
        oneOffCost: 0,
        monthlyCost: 0.38,
        costMonths: 72,
        axes: { marketVsPublic: 0.5, climateAmbition: -0.5, fiscalRestraint: -0.4 },
        salience: { marketVsPublic: 0.8, climateAmbition: 0.7 },
        effects: [effect({ target: 'businessSites', expected: 70, delayMonths: 6, rampMonths: 14, confidence: 'low' })],
        sourceIds: MODEL,
      },
      {
        id: 'mob-backlash-hold',
        label: 'Bei der Gebühr bleiben',
        rationale: 'Die Einnahmen bleiben, der Streit auch.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { climateAmbition: 0.7, marketVsPublic: -0.5 },
        salience: { climateAmbition: 0.8 },
        effects: [effect({ target: 'transitCapacity', expected: 70, delayMonths: 8, rampMonths: 16, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Anwohnerparken: moderat ════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'mob-parking-shortfall',
    kind: 'decision',
    category: 'finance',
    title: 'Die Parkraumbewirtschaftung trägt sich nicht',
    briefing:
      'Die moderate Gebühr deckt die Kosten der Bewirtschaftung nicht — Automaten, Kontrolle, Verwaltung. Das Rechnungsprüfungsamt nennt es eine Subvention und will sie benannt wissen.',
    urgency: 'normal',
    trigger: { earliestMonth: 24, latestMonth: 122, conditions: [], baseWeight: 7, cooldownMonths: 36, oncePerCampaign: true, requiresChoiceIds: ['mob-parking-reform:mob-parking-moderate'] },
    immediateEffects: [],
    defaultOptionId: 'mob-shortfall-accept',
    options: [
      {
        id: 'mob-shortfall-raise',
        label: 'Doch auf Kostendeckung anheben',
        rationale: 'Was beim ersten Mal nicht ging, geht jetzt — mit derselben Debatte.',
        oneOffCost: 0,
        monthlyCost: -0.42,
        axes: { fiscalRestraint: 0.7, climateAmbition: 0.5, marketVsPublic: -0.3 },
        salience: { fiscalRestraint: 0.8, climateAmbition: 0.6 },
        effects: [effect({ target: 'transitCapacity', expected: 60, delayMonths: 8, rampMonths: 16, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'mob-shortfall-accept',
        label: 'Als Subvention ausweisen',
        rationale: 'Ehrlich im Haushalt, und sie steht dort jedes Jahr.',
        oneOffCost: 0,
        monthlyCost: 0.17,
        costMonths: 108,
        axes: { fiscalRestraint: -0.5, marketVsPublic: 0.3 },
  ...
  /* ══ Weiche Erbbaurecht: vergeben ══════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'hou-leasehold-more',
    kind: 'decision',
    category: 'housing',
    title: 'Die Genossenschaften wollen mehr Grundstücke',
    briefing:
      'Auf den drei Erbbaugrundstücken stehen die ersten Häuser, und sie sind belegt. Vier weitere Flächen stehen zur Vergabe, und die Kämmerei rechnet weiterhin lieber mit Verkaufserlösen.',
    urgency: 'normal',
    trigger: { earliestMonth: 40, latestMonth: 122, conditions: [], baseWeight: 8, cooldownMonths: 38, oncePerCampaign: false, requiresChoiceIds: ['hou-leasehold:hou-leasehold-lease'] },
    immediateEffects: [],
    defaultOptionId: 'hou-more-two',
    options: [
      {
        id: 'hou-more-all',
        label: 'Alle vier im Erbbaurecht',
        rationale: 'Kein Erlös, dafür vierhundert gebundene Wohnungen und ein Erbbauzins für neunzig Jahre.',
        oneOffCost: 1.4,
        monthlyCost: -0.14,
        axes: { marketVsPublic: -0.9, redistribution: 0.8, fiscalRestraint: -0.6 },
        salience: { marketVsPublic: 1, redistribution: 0.8 },
        effects: [effect({ target: 'socialUnits', expected: 400, delayMonths: 16, rampMonths: 26, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'hou-more-two',
        label: 'Zwei vergeben, zwei verkaufen',
        rationale: 'Die Hälfte gebunden, die andere Hälfte im Haushalt.',
        oneOffCost: -7,
        monthlyCost: -0.06,
        axes: { marketVsPublic: -0.2, fiscalRestraint: 0.4 },
        salience: { marketVsPublic: 0.6, fiscalRestraint: 0.6 },
        effects: [effect({ target: 'socialUnits', expected: 190, delayMonths: 16, rampMonths: 24, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Erbbaurecht: verkauft ══════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'hou-land-gone',
    kind: 'decision',
    category: 'housing',
    title: 'Der Stadt gehört kein Bauland mehr',
    briefing:
      'Die Liegenschaftsverwaltung meldet: nach den letzten Verkäufen hat die Stadt im Innenbereich keine Fläche mehr, auf der sie selbst bauen oder vergeben könnte. Ankauf ist möglich, zu Marktpreisen.',
    urgency: 'important',
    trigger: { earliestMonth: 44, latestMonth: 124, conditions: [], baseWeight: 8, cooldownMonths: 48, oncePerCampaign: true, requiresChoiceIds: ['hou-leasehold:hou-leasehold-sell'] },
    immediateEffects: [],
    defaultOptionId: 'hou-land-none',
    options: [
      {
        id: 'hou-land-buy',
        label: 'Bodenfonds auflegen und zurückkaufen',
        rationale: 'Zwanzig Millionen, um wieder handlungsfähig zu werden. Zu Preisen, die die Stadt mitgemacht hat.',
        oneOffCost: 20,
        monthlyCost: 0.2,
        costMonths: 96,
        axes: { marketVsPublic: -0.8, fiscalRestraint: -0.8, redistribution: 0.6 },
        salience: { marketVsPublic: 0.9, fiscalRestraint: 0.9 },
        effects: [effect({ target: 'unitsUnderConstruction', expected: 220, delayMonths: 18, rampMonths: 24, confidence: 'low' })],
        sourceIds: MODEL,
      },
      {
        id: 'hou-land-none',
        label: 'Ohne eigene Flächen auskommen',
        rationale: 'Gebaut wird trotzdem — nur nicht mehr von der Stadt und nicht mehr zu ihren Bedingungen.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { marketVsPublic: 0.8, fiscalRestraint: 0.7 },
        salience: { marketVsPublic: 0.9 },
        effects: [],
        sourceIds: MODEL,
      },
  ...
  /* ══ Weiche Zuweisungen: dezentral ═════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'soc-decentral-school',
    kind: 'decision',
    category: 'social',
    title: 'Vier Quartiere, vier überfüllte Grundschulen',
    briefing:
      'Die dezentrale Unterbringung hat funktioniert — und die vier Sprengel haben jetzt je zwei Klassen zu viel. Container gingen schnell, ein Anbau geht richtig.',
    urgency: 'important',
    trigger: { earliestMonth: 20, latestMonth: 124, conditions: [{ metric: 'schoolUtilisation', operator: '>', value: 96 }], baseWeight: 9, cooldownMonths: 30, oncePerCampaign: false, requiresChoiceIds: ['soc-allocation:soc-allocation-decentral'] },
    immediateEffects: [],
    defaultOptionId: 'soc-decentral-container',
    options: [
      {
        id: 'soc-decentral-extend',
        label: 'Vier Anbauten',
        rationale: 'Drei Jahre Bauzeit, danach stehen sie fünfzig.',
        oneOffCost: 19,
        monthlyCost: 0.44,
        axes: { redistribution: 0.6, fiscalRestraint: -0.7, opennessIntegration: 0.5 },
        salience: { redistribution: 0.7, fiscalRestraint: 0.8 },
        effects: [effect({ target: 'schoolPlaces', expected: 480, delayMonths: 26, rampMonths: 12, confidence: 'high' })],
        sourceIds: MODEL,
      },
      {
        id: 'soc-decentral-container',
        label: 'Container aufstellen',
        rationale: 'In vier Monaten da, und sie stehen länger, als irgendjemand plant.',
        oneOffCost: 4.6,
        monthlyCost: 0.26,
        costMonths: 120,
        axes: { fiscalRestraint: 0.5, redistribution: 0.2 },
        salience: { fiscalRestraint: 0.7 },
        effects: [effect({ target: 'schoolPlaces', expected: 320, delayMonths: 5, rampMonths: 6, confidence: 'high' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Zuweisungen: zentral ═══════════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'soc-central-pressure',
    kind: 'decision',
    category: 'social',
    title: 'Die Sammelunterkunft platzt',
    briefing:
      'Sechshundert Menschen an einem Ort, ausgelegt für vierhundert. Der Betreiber kündigt zum Jahresende, die Aufsicht beanstandet die Sanitäranlagen, und im Umfeld häufen sich die Einsätze.',
    urgency: 'breaking',
    trigger: { earliestMonth: 20, latestMonth: 124, conditions: [], baseWeight: 10, cooldownMonths: 28, oncePerCampaign: false, requiresChoiceIds: ['soc-allocation:soc-allocation-central'] },
    immediateEffects: [],
    defaultOptionId: 'soc-central-patch',
    options: [
      {
        id: 'soc-central-split',
        label: 'Auf vier Standorte verteilen',
        rationale: 'Was am Anfang abgelehnt wurde, jetzt unter Zeitdruck und teurer.',
        oneOffCost: 11,
        monthlyCost: 0.52,
        axes: { opennessIntegration: 0.8, redistribution: 0.6, fiscalRestraint: -0.6 },
        salience: { opennessIntegration: 0.9 },
        effects: [effect({ target: 'integrationPlaces', expected: 380, delayMonths: 8, rampMonths: 16, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'soc-central-patch',
        label: 'Sanitär nachrüsten und weitermachen',
        rationale: 'Die Beanstandung ist erledigt. Die Belegung nicht.',
        oneOffCost: 3.4,
        monthlyCost: 0,
        axes: { fiscalRestraint: 0.6, opennessIntegration: -0.4, securityAuthority: 0.4 },
        salience: { fiscalRestraint: 0.7, opennessIntegration: 0.6 },
        effects: [effect({ target: 'integrationPlaces', expected: 60, delayMonths: 4, rampMonths: 8, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 2,
  ...
  /* ══ Weiche Solarpflicht: beschlossen ══════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'env-solar-grid',
    kind: 'decision',
    category: 'environment',
    title: 'Der eigene Strom will verkauft werden',
    briefing:
      'Neunzig Dächer liefern mehr, als die Liegenschaften selbst verbrauchen. Einspeisen bringt den garantierten Satz; ein Mieterstrommodell brächte mehr und wäre Verwaltungsarbeit für Jahre.',
    urgency: 'normal',
    trigger: { earliestMonth: 22, latestMonth: 124, conditions: [], baseWeight: 7, cooldownMonths: 40, oncePerCampaign: true, requiresChoiceIds: ['env-solar-roofs:env-solar-build'] },
    immediateEffects: [],
    defaultOptionId: 'env-grid-feed',
    options: [
      {
        id: 'env-grid-tenants',
        label: 'Mieterstrom für die städtischen Wohnungen',
        rationale: 'Mehr Ertrag, geringere Nebenkosten, und eine Abrechnung, die jemand machen muss.',
        oneOffCost: 2.8,
        monthlyCost: 0.14,
        costMonths: 120,
        axes: { climateAmbition: 0.7, redistribution: 0.7, marketVsPublic: -0.6 },
        salience: { climateAmbition: 0.8, redistribution: 0.7 },
        effects: [effect({ target: 'cleanHeat', expected: 5.5, delayMonths: 8, rampMonths: 18, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'env-grid-feed',
        label: 'Einfach einspeisen',
        rationale: 'Der garantierte Satz, kein Personal, keine Abrechnung.',
        oneOffCost: 0,
        monthlyCost: -0.11,
        axes: { climateAmbition: 0.4, fiscalRestraint: 0.5, marketVsPublic: 0.3 },
        salience: { climateAmbition: 0.6, fiscalRestraint: 0.6 },
        effects: [effect({ target: 'cleanHeat', expected: 2.2, delayMonths: 4, rampMonths: 10, confidence: 'high' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Zweckentfremdung: durchgesetzt ═════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'hou-vacancy-staff',
    kind: 'decision',
    category: 'housing',
    title: 'Die Satzung braucht Leute, die sie anwenden',
    briefing:
      'Zwölfhundert Verdachtsfälle, drei Stellen im Wohnungsamt und eine Bearbeitungszeit von vierzehn Monaten. Ohne Personal ist die Satzung ein Papier, mit Personal ein Eingriff, der Widerspruch erzeugt.',
    urgency: 'normal',
    trigger: { earliestMonth: 22, latestMonth: 122, conditions: [], baseWeight: 8, cooldownMonths: 34, oncePerCampaign: true, requiresChoiceIds: ['hou-vacancy-levy:hou-vacancy-enforce'] },
    immediateEffects: [],
    defaultOptionId: 'hou-vacancy-thin',
    options: [
      {
        id: 'hou-vacancy-team',
        label: 'Neun Stellen für den Vollzug',
        rationale: 'Dann wirkt sie. Und dann merkt man sie auch.',
        oneOffCost: 0.6,
        monthlyCost: 0.58,
        axes: { marketVsPublic: -0.7, redistribution: 0.6, securityAuthority: 0.4 },
        salience: { marketVsPublic: 0.8, redistribution: 0.7 },
        effects: [effect({ target: 'housingUnits', expected: 340, delayMonths: 10, rampMonths: 20, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'hou-vacancy-thin',
        label: 'Mit drei Stellen weitermachen',
        rationale: 'Die Satzung steht im Amtsblatt. Mehr passiert nicht.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { fiscalRestraint: 0.7, marketVsPublic: 0.3 },
        salience: { fiscalRestraint: 0.8 },
        effects: [effect({ target: 'housingUnits', expected: 60, delayMonths: 12, rampMonths: 18, confidence: 'low' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Bürgerhaushalt: eingeführt ═════════════════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'gov-budget-verdict',
    kind: 'decision',
    category: 'governance',
    title: 'Der Bürgerhaushalt hat abgestimmt — gegen die Verwaltung',
    briefing:
      'Zwei Millionen standen zur Verfügung. Die Bürgerinnen wollen davon Spielplätze und einen Skatepark; die Verwaltung hatte Radabstellanlagen und Schulhofsanierung vorbereitet. Beides geht nicht.',
    urgency: 'normal',
    trigger: { earliestMonth: 22, latestMonth: 124, conditions: [], baseWeight: 8, cooldownMonths: 30, oncePerCampaign: false, requiresChoiceIds: ['gov-citizen-budget:gov-citizen-adopt'] },
    immediateEffects: [],
    defaultOptionId: 'gov-verdict-follow',
    options: [
      {
        id: 'gov-verdict-follow',
        label: 'Dem Votum folgen',
        rationale: 'Genau dafür war es da. Die Fachämter lernen es zu ertragen.',
        oneOffCost: 2,
        monthlyCost: 0,
        axes: { redistribution: 0.5, marketVsPublic: -0.4, growthVsPreservation: -0.3 },
        salience: { redistribution: 0.7 },
        effects: [effect({ target: 'greenSpaceHectares', expected: 3, delayMonths: 10, rampMonths: 14, confidence: 'high' })],
        sourceIds: MODEL,
      },
      {
        id: 'gov-verdict-override',
        label: 'Der Fachplanung folgen',
        rationale: 'Sachlich besser begründet — und der Bürgerhaushalt war beim nächsten Mal ein Vorschlagswesen.',
        oneOffCost: 2,
        monthlyCost: 0,
        axes: { marketVsPublic: 0.2, securityAuthority: 0.5, climateAmbition: 0.4 },
        salience: { securityAuthority: 0.6, climateAmbition: 0.5 },
        effects: [effect({ target: 'transitCapacity', expected: 40, delayMonths: 8, rampMonths: 12, confidence: 'medium' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
  ...
  /* ══ Weiche Sanierungsstau: angegangen oder nicht ══════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'fin-maintenance-closure',
    kind: 'decision',
    category: 'finance',
    title: 'Zwei Turnhallen werden gesperrt',
    briefing:
      'Der Prüfstatiker hat entschieden, nicht der Rat: zwei Hallen sind ab sofort zu. Vereine und Schulsport stehen ohne da, und die Sanierung kostet das Dreifache dessen, was die Wartung gekostet hätte.',
    urgency: 'breaking',
    trigger: { earliestMonth: 30, latestMonth: 128, conditions: [{ metric: 'investmentBacklog', operator: '>', value: 90 }], baseWeight: 10, cooldownMonths: 30, oncePerCampaign: false, requiresRefusedEventIds: ['fin-maintenance-program'] },
    immediateEffects: [],
    defaultOptionId: 'fin-closure-wait',
    options: [
      {
        id: 'fin-closure-fix',
        label: 'Beide sanieren',
        rationale: 'Das Dreifache, jetzt, und danach stehen sie wieder dreißig Jahre.',
        oneOffCost: 16.5,
        monthlyCost: 0,
        axes: { fiscalRestraint: -0.7, redistribution: 0.5 },
        salience: { fiscalRestraint: 0.9 },
        effects: [effect({ target: 'maintenanceSpend', expected: 0.9, delayMonths: 4, rampMonths: 14, confidence: 'high' })],
        sourceIds: MODEL,
      },
      {
        id: 'fin-closure-wait',
        label: 'Gesperrt lassen',
        rationale: 'Kostet heute nichts. Die Vereine fahren in den Nachbarkreis.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { fiscalRestraint: 0.9 },
        salience: { fiscalRestraint: 1 },
        effects: [effect({ target: 'greenSpaceHectares', expected: -2, delayMonths: 6, rampMonths: 12, confidence: 'low' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 2,
  ...
  /* ══ Weiche Hafenausbau: die Gegenseite meldet sich ════════════════════════════════════════════ */
  {
    schemaVersion: 1,
    id: 'eco-terminal-automation',
    kind: 'decision',
    category: 'economy',
    title: 'Das Terminal soll automatisiert werden',
    briefing:
      'Zwei Jahre nach Inbetriebnahme legt der Betreiber einen Automatisierungsplan vor: doppelter Umschlag, ein Drittel weniger Personal. Die Stadt hält Anteile und sitzt im Aufsichtsrat.',
    urgency: 'important',
    trigger: { earliestMonth: 52, latestMonth: 126, conditions: [], baseWeight: 8, cooldownMonths: 44, oncePerCampaign: true, requiresChoiceIds: ['eco-harbour-expansion:eco-harbour-expand'] },
    immediateEffects: [],
    defaultOptionId: 'eco-automation-yes',
    options: [
      {
        id: 'eco-automation-yes',
        label: 'Zustimmen',
        rationale: 'Der Umschlag verdoppelt sich, zweihundert Stellen fallen weg.',
        oneOffCost: 0,
        monthlyCost: -0.34,
        axes: { marketVsPublic: 0.7, growthVsPreservation: 0.7, redistribution: -0.6 },
        salience: { marketVsPublic: 0.8, redistribution: 0.8 },
        effects: [effect({ target: 'businessSites', expected: 120, delayMonths: 12, rampMonths: 20, confidence: 'medium' })],
        sourceIds: MODEL,
      },
      {
        id: 'eco-automation-condition',
        label: 'Nur mit Beschäftigungssicherung',
        rationale: 'Keine betriebsbedingten Kündigungen für acht Jahre. Der Betreiber rechnet neu.',
        oneOffCost: 0,
        monthlyCost: 0,
        axes: { redistribution: 0.8, marketVsPublic: -0.6, growthVsPreservation: -0.2 },
        salience: { redistribution: 0.9, marketVsPublic: 0.7 },
        effects: [effect({ target: 'businessSites', expected: 45, delayMonths: 14, rampMonths: 22, confidence: 'low' })],
        sourceIds: MODEL,
      },
    ],
    expiresInMonths: 3,
