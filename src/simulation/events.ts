...
export function applyMeasures(
  ...
  for (const measure of measures) {
    ...
    for (const effect of measure.effects) {
      ...
      if (isStock(effect.target)) stocks[effect.target] += delta
...
function exceedance(metrics: CityMetrics, event: EventDefinition): number {
  if (event.trigger.conditions.length === 0) return 1
  let total = 0
...
export function eligibleEvents(state: EventDrawState, monthOfYear: number): EventDefinition[] {
  return EVENTS.filter((event) => {
    ...
    if (state.month < trigger.earliestMonth || state.month > trigger.latestMonth) return false
    if (trigger.oncePerCampaign && state.firedOnce.includes(event.id)) return false
    if ((state.cooldowns[event.id] ?? 0) > state.month) return false
    if (trigger.scheduledMonthOfYear !== undefined && trigger.scheduledMonthOfYear !== monthOfYear) return false
    if (trigger.requiresEventIds?.some((id) => !state.firedOnce.includes(id))) return false
    if (trigger.blockedByMeasureIds?.some((id) => state.activeMeasureSources.includes(id))) return false
    if (event.options.length > 0 && state.openDecisions >= 2) return false
    for (const condition of trigger.conditions) {
      if (!conditionHolds(state.metrics, condition.metric, condition.operator, condition.value)) return false
      if (condition.sustainedMonths && (state.streaks[event.id] ?? 0) < condition.sustainedMonths) return false
    }
    return true
  })
}
...
export function drawEvent(state: EventDrawState, monthOfYear: number, stream: RandomStream): EventDefinition | null {
  ...
  if (stream.next() > 0.8) return null
  ...
  for (let index = 0; index < eligible.length; index += 1) {
    ...
    if (roll <= 0) return eligible[index] ?? null
  }
