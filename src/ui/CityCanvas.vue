...
onMounted(async () => {
  ...
  if (snapshot.value) cityRenderer.applySnapshot(snapshot.value)
})

watch(snapshot, (next) => {
  if (next) cityRenderer?.applySnapshot(next)
})

watch(() => game.selectedBuilding?.id, (buildingId) => {
  if (buildingId) cityRenderer?.focusBuilding(buildingId)
})
...
<template>
  <canvas ref="canvas" class="city-canvas" aria-label="Interaktive 3D-Stadt Lindenhafen"></canvas>
</template>
