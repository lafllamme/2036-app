...
const surprise = computed(() => {
  ...
  if (result.passed && chance < 0.4) return 'Das war enger als prognostiziert – die Vorlage ist trotz schwacher Aussicht durchgekommen.'
  if (!result.passed && chance > 0.6) return 'Trotz guter Prognose gescheitert: einzelne Fraktionen sind abgesprungen.'
  return null
})
</script>
...
<template>
  <div v-if="lastVoteResult" class="modal-backdrop" @click.self="game.dismissVoteResult()">
    <article class="vote-result panel" role="dialog" aria-modal="true" aria-labelledby="result-title">
      <button type="button" class="close-button" aria-label="Ergebnis schließen" @click="game.dismissVoteResult()">×</button>
      ...
      <p v-if="surprise" class="surprise">{{ surprise }}</p>
      <ul class="vote-rows">
        <li v-for="row in rows" :key="row.partyId" :class="row.vote">
          <i :style="{ background: row.party?.color }"></i>
          <span>{{ row.party?.abbreviation }} · {{ row.seats }}</span>
          <b>{{ row.label }}</b>
        </li>
      </ul>
      <button type="button" class="primary" @click="game.dismissVoteResult()">Weiter</button>
    </article>
  </div>
</template>
