...
watch(openDecision, (next) => {
  if (next) game.requestForecasts(next.definition.id)
}, { immediate: true })
...
function seatSplit(optionId: string): { yes: number; no: number; abstain: number } {
  ...
  if (!forecast) return { yes: 0, no: 0, abstain: COUNCIL_SEATS }
...
function outlook(optionId: string): { word: string; tone: 'positive' | 'pending' | 'negative' } {
  ...
  if (value >= 70) return { word: 'geht durch', tone: 'positive' }
  if (value >= 40) return { word: 'offener Ausgang', tone: 'pending' }
...
function stance(optionId: string, partyId: PartyId): { label: string; tone: string } {
  ...
  if (!party) return { label: '–', tone: 'open' }
  if (party.probabilities.yes > 0.6) return { label: 'dafür', tone: 'yes' }
  if (party.probabilities.no > 0.6) return { label: 'dagegen', tone: 'no' }
...
function blockers(optionId: string): string | null {
  const forecast = forecastOf(optionId)
  if (!forecast || chance(optionId) >= 70) return null
...
const gains = (option: EventOption): PolicyEffect[] =>
  option.effects.filter((effect) => effectTone(effect.target, effect.expected) === 'gain')

const losses = (option: EventOption): PolicyEffect[] =>
  option.effects.filter((effect) => effectTone(effect.target, effect.expected) === 'loss')
...
<template>
  <div v-if="openDecision" class="modal-backdrop" @click.self="game.openDecisionSheet(null)">
    <article class="vote-sheet panel" role="dialog" aria-modal="true" aria-labelledby="vote-title">
      <button type="button" class="close-button" aria-label="Vorlage schließen" @click="game.openDecisionSheet(null)">×</button>

      <header>
        <small>{{ CATEGORY_LABELS[openDecision.definition.category] }} · Ratsvorlage</small>
        <h2 id="vote-title">{{ openDecision.definition.title }}</h2>
      ...
      <section v-for="option in openDecision.definition.options" :key="option.id" class="vote-option">
        <div class="vote-option__head">
          <h3 v-if="!singleOption">{{ option.label }}</h3>
          <h3 v-else class="vote-option__head--implicit">Beschlussvorschlag</h3>
          <span class="outlook" :class="outlook(option.id).tone">{{ outlook(option.id).word }}</span>
        </div>
        <p v-if="!singleOption" class="rationale">{{ option.rationale }}</p>
        ...
        <div class="ledger">
          <div class="ledger__column gain">
            ...
            <p v-else class="ledger__empty">Nichts – diese Option baut keine Kapazität auf.</p>
          </div>
          ...
          <div class="ledger__column risk">
            ...
            <ul v-if="risks(option, option.id).length > 0">
              <li v-for="entry in risks(option, option.id)" :key="entry"><span>{{ entry }}</span></li>
            </ul>
            <p v-else class="ledger__empty">Keine erkennbaren Nebenwirkungen.</p>
          </div>
        </div>
        ...
        <div class="forecast">
          ...
          <div class="seat-bar" role="img" :aria-label="`${seatSplit(option.id).yes} Ja, ${seatSplit(option.id).abstain} Enthaltungen, ${seatSplit(option.id).no} Nein von 60 Sitzen`">
            <i class="yes" :style="{ width: `${(seatSplit(option.id).yes / 60) * 100}%` }"></i>
            <i class="abstain" :style="{ width: `${(seatSplit(option.id).abstain / 60) * 100}%` }"></i>
            <i class="no" :style="{ width: `${(seatSplit(option.id).no / 60) * 100}%` }"></i>
          </div>
          <p v-if="blockers(option.id)" class="blockers">{{ blockers(option.id) }}</p>
      ...
      <section class="negotiation">
        ...
        <div class="negotiation__row">
          ...
          >
            <i :style="{ background: party.color }"></i>{{ party.abbreviation }}
            <template v-if="openDecision.prepared.negotiatedPartyIds.includes(party.id)">✓</template>
          </button>
        </div>
      </section>
