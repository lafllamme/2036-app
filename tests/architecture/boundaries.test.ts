...
describe('architecture boundaries', () => {
  ...
  it('never references a design token that does not exist', () => {
    ...
    /*
     * Was im `:style` einer Komponente entsteht und deshalb nie im Token-Blatt steht. `--weight` ist
     * der Mandatsanteil einer Fraktion in der Parteienhalle: aus ihm folgt die Schriftgröße ihrer
     * Zeile, damit die stärkste Fraktion auch die größte ist.
     */
    const runtimeProvided = new Set(['--party-color', '--party-accent', '--tone', '--call', '--weight'])
